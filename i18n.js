...
const T = {
  en: {
    ...
    gapNote: n => `Indicative only. Typical bill = published medians from ${n} HK private hospital(s); the range spans hospitals. Modelled as: eligible charge → per-item cap → coinsurance → deductible → annual limit, assuming a 2-day stay. The insurer assesses each claim on the actual itemised bill.`,
  ...
  hk: {
    ...
    gapNote: n => `僅供參考。一般帳單取自 ${n} 間香港私家醫院公布的中位數，範圍涵蓋各醫院。計算方式：合資格費用 → 各項賠償上限 → 共同保險 → 自付費 → 每年保障限額，假設住院 2 日。實際賠償由保險公司按帳單逐項審核。`,
  ...
  cn: {
    ...
    gapNote: n => `仅供参考。一般账单取自 ${n} 间香港私家医院公布的中位数，范围涵盖各医院。计算方式：合资格费用 → 各项赔偿上限 → 共同保险 → 自付费 → 每年保障限额，假设住院 2 日。实际赔偿由保险公司按账单逐项审核。`,
